...
function loader(elemento_retorno){
	...
	$(elemento_retorno).html(
		'<div style="width:100%;margin:75px auto">' +
			'<center>' +
				'<img width="32" align="middle" src="'+session.urlsystem+'system/tema/padrao/loading.gif">' +
	...
	switch (tipo){
		case 'success':
			if (!$(campo).parent().hasClass("calendar-picker-group")){
				...
				$(campo).parent().append(
					'<span class=\"glyphicon glyphicon-ok form-control-feedback status-'+$(campo).attr("id")+'\" aria-hidden=\"true\"></span>' +
		...
		case 'error':
			if (!$(campo).parent().hasClass("calendar-picker-group")){
				...
				$(campo).parent().append(
					'<span class=\"glyphicon glyphicon-remove form-control-feedback status-'+$(campo).attr("id")+'\" aria-hidden=\"true\"></span>' +
...
function getURLProject(parametro = null){
	...
	var urlproject 		= session.urlsystem + "index.php";
	var parmsProject 	= [];
...
function getSRCLoader(){
	var src = session.urlsystem + 'system/tema/padrao/loading2.gif';
	return src;
}
